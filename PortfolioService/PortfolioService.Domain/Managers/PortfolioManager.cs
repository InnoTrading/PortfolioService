...
namespace PortfolioService.Domain.Managers
{
    public class PortfolioManager(IUnitOfWork unitOfWork) : IPortfolioManager
    {
        ...
        {
            var existingStocks =
                await _unitOfWork.UserStocks.GetByConditionAsync(s => s.UserID == userId && s.StockID == stockId);
        ...
        public async Task<bool> RemoveStocks(Guid userId, Guid stockId, int quantityToSell)
        {
